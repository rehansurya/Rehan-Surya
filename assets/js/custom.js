...
const progress_number = $('.loading-count'),
      preloader_progress = $('.dsn-progress-path'),
      present = {
          value: 0
      };
...
// Get the photo containers
const lightModePhotos = document.querySelectorAll('.v-light-photo');
const darkModePhotos = document.querySelectorAll('.v-dark-photo');

// Function to toggle the photo visibility based on the mode
function togglePhotoVisibility() {
  if (body.classList.contains('v-dark')) {
    // Dark mode is enabled, show dark mode photos and hide light mode photos
    lightModePhotos.forEach(photo => {
      photo.style.display = 'none';
    });
    darkModePhotos.forEach(photo => {
      photo.style.display = 'block';
    });
  } else {
    // Light mode is enabled, show light mode photos and hide dark mode photos
    lightModePhotos.forEach(photo => {
      photo.style.display = 'block';
    });
    darkModePhotos.forEach(photo => {
...
// Add an event listener to toggle the photo visibility when the mode changes
body.addEventListener('click', function() {
  // Toggle the "v-dark" class on the body element
  body.classList.toggle('v-dark-photo');

  // Toggle the photo visibility based on the mode
  togglePhotoVisibility();
});

// Swiper slide change event listener
const swiper = document.querySelector('.dsn-swiper .swiper-container').swiper;
